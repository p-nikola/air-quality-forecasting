...
OFFLINE_DIR = SKOPJE_DIR / "offline-Phase"
DEFAULT_CONTEXT_PATH = Path(__file__).resolve().parent / "context_skopje.csv"
CONTEXT_PATH = Path(os.getenv("SKOPJE_CONTEXT_CSV_PATH", str(DEFAULT_CONTEXT_PATH))).expanduser()
DEFAULT_FORECAST_PATH = SKOPJE_DIR / "data" / "raw" / "skopje_forecast_weather.csv"
FORECAST_PATH = Path(os.getenv("SKOPJE_FORECAST_CSV_PATH", str(DEFAULT_FORECAST_PATH))).expanduser()
ZERO_SHOT_DIR = Path(
    os.getenv("SKOPJE_ZERO_SHOT_MODEL_PATH", str(OFFLINE_DIR / "chronos2_zero_shot"))
).expanduser()
PREDICTION_HOURS = 24
CONTEXT_CITY = os.getenv("SKOPJE_CONTEXT_CITY", "skopje")
ZERO_SHOT_MIN_CONTEXT_POINTS = int(os.getenv("SKOPJE_ZERO_SHOT_MIN_CONTEXT_POINTS", "2"))

SENSOR_TOPIC_PREFIX = os.getenv("SKOPJE_SENSOR_TOPIC_PREFIX", "skopje_sensor_")
PM10_TOPIC = os.getenv("SKOPJE_PM10_TOPIC", "SkopjeFullPm10WeatherData")
PM25_TOPIC = os.getenv("SKOPJE_PM25_TOPIC", "SkopjeFullPm25WeatherData")
PM10_ZERO_SHOT_TOPIC = os.getenv("SKOPJE_PM10_ZERO_SHOT_TOPIC", "SkopjeFullPm10WeatherData_ZeroShot")
PM25_ZERO_SHOT_TOPIC = os.getenv("SKOPJE_PM25_ZERO_SHOT_TOPIC", "SkopjeFullPm25WeatherData_ZeroShot")
...
pm10_predictor = load_predictor(OFFLINE_DIR / "chronos2_model_pm10_skopje")
pm25_predictor = load_predictor(OFFLINE_DIR / "chronos2_model_pm25_skopje")
...
def append_neighbors(df_hourly, neighbors_df, weather_cols=["humidity", "pressure", "temperature", "wind_speed"], k_search=20, k_keep=5):
    ...
    # Create the 1..k rank for the wide-format columns
...
NEIGHBORS_PATH = SKOPJE_DIR / "data" / "neighbors_data" / "skopje_neighbors.csv"
...
def load_forecast():
    ...
    forecast_df = normalize_weather_columns(forecast_df)
    forecast_df["timestamp"] = pd.to_datetime(forecast_df["timestamp"], utc=True)
    forecast_df["sensorId"] = forecast_df["sensorId"].astype(str)
    forecast_df = forecast_df.drop(columns=["lat", "lon", "wind_direction_10m"], errors="ignore")
...
df = (
    ...
    .option("subscribePattern", f"{SENSOR_TOPIC_PREFIX}.*")
    .load()
)
